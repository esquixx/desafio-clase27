...
export default class appRouter {
    ...
    generateCustomResponses = (req, res, next) => {
        ...
        res.authFailError = (error) => res.status(401).json({ status: 'error', error })
        res.sendRequestError = (error) => res.status(404).json({ status: 'error', error })
        next()
    }
    ...
    handlePolicies = policies => (req, res, next) => {
        ...
        if (!authHeaders) return res.status(401).render('errors/errorPage', { status: 'error', error: 'Unauthorized' })
        const token = cookieExtractor(req)
        let user = jwt.verify(token, PRIVATE_KEY)
        if (!policies.includes(user.user.role.toUpperCase())) return res.status(403).render('errors/errorPage', { status: 'error', error: 'No authorized' })
        req.user = user
        next()
    }
}